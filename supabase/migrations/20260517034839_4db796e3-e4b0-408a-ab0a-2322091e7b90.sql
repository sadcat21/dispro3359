...
BEGIN
  ...
  v_anchor := COALESCE(v_session_end, '1970-01-01'::timestamptz);
  ...
  LOOP
    ...
    -- Add back the gift boxes for pending (un-confirmed) deferred offer cards.
    -- This covers the case where a delivery was recorded as two separate
    -- stock_movements (paid + gift) so the per-movement deferred check above
    -- could not detect the gift portion. The gift must stay in the truck
    -- until the worker confirms the pending_offer_confirmation card.
    SELECT COALESCE(SUM(
    ...
    WHERE poc.worker_id = p_worker_id
      ...
      -- Only when there is NO separate confirmation movement (which would
      -- already have deducted the gift): we must not double-add the gift back.
      AND NOT EXISTS (
