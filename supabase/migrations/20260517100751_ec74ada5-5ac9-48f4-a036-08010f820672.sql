...
BEGIN
  ...
  LOOP
    ...
    -- Pending deferred gifts are already left in truck stock because their
    -- delivery movement excludes them. Do not add pending cards again; doing so
    -- inflates balances such as 8.10 into 9.02 when two pending pieces exist.
    ...
    LEFT JOIN promos promo ON sm.reference_type = 'promo' AND promo.id = promo.reference_id
    LEFT JOIN orders promo_order ON promo_order.id = promo.order_id
    LEFT JOIN pending_offer_confirmations poc ON sm.reference_type = 'pending_offer_confirmation' AND poc.id = sm.reference_id
    LEFT JOIN orders pending_order ON pending_order.id = poc.order_id
...
-- Remove duplicate pending cards for the same order/product/offer/status while keeping the newest card that has an order_item_id when possible.
WITH ranked AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY order_id, product_id, offer_id, status
           ORDER BY (order_item_id IS NOT NULL) DESC, created_at DESC, id DESC
         ) AS rn
