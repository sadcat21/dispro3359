-- 1) Patch confirm_pending_offer: when inserting a fresh sales_tracking row,
--    include the paid portion (purchased - gift), not only the gift.
...
BEGIN
  ...
  ELSE
    -- Recover the paid (sold) portion if recordSaleTracking did not run earlier.
    v_paid_pieces_for_insert := GREATEST(
      0,
      GREATEST(0, COALESCE(rec.purchased_boxes, 0)) * v_ppb
...
-- 2) Backfill past confirmed pending offers whose sales_tracking row never got
--    a paid portion. For each such row, add (purchased - gift) into sold_boxes/pieces.
WITH targets AS (
  SELECT
    st.id           AS st_id,
    GREATEST(1, COALESCE(st.pieces_per_box, poc.pieces_per_box, 1)) AS ppb,
    GREATEST(0, COALESCE(poc.purchased_boxes, 0))  AS p_boxes,
    GREATEST(0, COALESCE(poc.purchased_pieces, 0)) AS p_pieces,
    GREATEST(0, COALESCE(poc.gift_boxes, 0))       AS g_boxes,
    GREATEST(0, COALESCE(poc.gift_pieces, 0))      AS g_pieces
  FROM public.pending_offer_confirmations poc
  JOIN public.sales_tracking st
    ON st.order_id = poc.order_id
   AND st.product_id = COALESCE(poc.gift_product_id, poc.product_id)
   AND st.source = CASE WHEN poc.source IN ('direct_sale','delivery_sale','warehouse_sale') THEN poc.source ELSE 'delivery_sale' END
  WHERE poc.status = 'confirmed'
    AND poc.order_id IS NOT NULL
    AND COALESCE(st.sold_boxes, 0) = 0
    AND COALESCE(st.sold_pieces, 0) = 0
    AND (COALESCE(poc.purchased_boxes, 0) > 0 OR COALESCE(poc.purchased_pieces, 0) > 0)
), calc AS (
  SELECT
    st_id,
    ppb,
    GREATEST(0, (p_boxes * ppb + p_pieces) - (g_boxes * ppb + g_pieces)) AS paid_pieces
  FROM targets
)
UPDATE public.sales_tracking st
SET sold_boxes  = c.paid_pieces / c.ppb,
    sold_pieces = c.paid_pieces - (c.paid_pieces / c.ppb) * c.ppb,
    notes = COALESCE(st.notes, '') || ' [backfilled paid portion]'
FROM calc c
WHERE st.id = c.st_id;