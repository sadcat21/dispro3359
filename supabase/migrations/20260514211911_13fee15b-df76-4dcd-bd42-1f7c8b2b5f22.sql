CREATE OR REPLACE FUNCTION public.stock_qty_bp_to_pieces(
  p_qty numeric,
  p_ppb integer
) RETURNS numeric
LANGUAGE sql
...
AS $$
  SELECT (FLOOR(ABS(COALESCE(p_qty, 0))) * GREATEST(COALESCE(p_ppb, 1), 1))
       + ROUND((ABS(COALESCE(p_qty, 0)) - FLOOR(ABS(COALESCE(p_qty, 0)))) * 100)
$$;
    ...
    WITH delivered_items AS (
      ...
      SELECT
        ...
        COALESCE(
          ...
          NULLIF((
            SELECT SUM(public.stock_qty_bp_to_pieces(sm.quantity, r.ppb))
            FROM stock_movements sm
      ...
      SELECT COALESCE(SUM(public.stock_qty_bp_to_pieces(sm.quantity, r.ppb)), 0) AS pieces
      FROM stock_movements sm
...
GRANT EXECUTE ON FUNCTION public.stock_qty_bp_to_pieces(numeric, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.preview_recalibrate_worker_stock(uuid) TO authenticated;
