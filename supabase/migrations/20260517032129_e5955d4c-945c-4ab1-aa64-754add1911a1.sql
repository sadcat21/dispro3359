...
BEGIN
  ...
  -- If this stock movement is the confirmation movement for a pending offer,
  -- treat that specific pending card as no longer pending during validation.
  v_current_pending_confirmation_id := CASE
    WHEN NEW.reference_type = 'pending_offer_confirmation' THEN NEW.reference_id
    ELSE NULL
  END;

  -- Total ordered pieces for this product on this order:
  -- order_items.quantity stores box.piece notation, full-box gifts are stored
  -- in gift_quantity, and piece-level gifts are stored separately in gift_pieces.
  SELECT COALESCE(SUM(
