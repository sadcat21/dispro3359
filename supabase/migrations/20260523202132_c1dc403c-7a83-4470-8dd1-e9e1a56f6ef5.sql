...
BEGIN
  ...
  LOOP
    -- Anchor on the most recent warehouse->worker confirmation (stock_movements load),
    -- falling back to the latest fresh loading session if no SM exists yet.
    SELECT MAX(sm.created_at) INTO v_fresh_load_sm
    FROM stock_movements sm
    WHERE sm.worker_id = p_worker_id AND sm.product_id = r.pid
      AND sm.movement_type = 'load'
      AND sm.created_at > v_acct_anchor;

    SELECT MAX(COALESCE(ls.completed_at, ls.created_at)) INTO v_fresh_load_ls
    ...
    v_fresh_load := COALESCE(v_fresh_load_sm, v_fresh_load_ls);
    ...
    -- Prefer stock_movements load total (reflects actual confirmed pieces after the anchor);
    -- fall back to loading_session_items if no SM exists.
