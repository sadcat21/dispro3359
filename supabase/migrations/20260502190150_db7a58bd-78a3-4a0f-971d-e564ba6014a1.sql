...
BEGIN
  ...
  SELECT id INTO v_actor_worker_id FROM public.workers WHERE user_id = v_actor_user_id LIMIT 1;
  ...
  LOOP
    ...
    -- ملاحظة: الهدية مُضمَّنة بالفعل في v_item.quantity المُدخلة من مسؤول المخزن.
    -- لا نضيفها مجددًا حتى لا يُضاعف رصيد العامل.
    v_total_load_pieces := (v_item_boxes * v_pieces_per_box) + v_item_piece_part;
