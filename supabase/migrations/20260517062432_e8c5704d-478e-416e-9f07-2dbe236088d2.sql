...
BEGIN
  ...
  LOOP
    ...
    -- Only count shipments the worker has APPROVED (status='completed').
    -- 'open' = manager still preparing, 'pending_confirmation' = awaiting
    -- worker approval, 'unloaded' = truck emptied — none of these should
    -- contribute to the worker's balance.
    SELECT COALESCE(SUM(
