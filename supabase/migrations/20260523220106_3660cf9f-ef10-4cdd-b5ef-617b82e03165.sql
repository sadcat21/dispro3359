...
BEGIN
  ...
  LOOP
    -- Fresh-load anchor: EARLIEST true-reset load after the accounting anchor,
    -- only if no other completed load exists strictly before it since the anchor.
    ...
    SELECT COALESCE(SUM(sm.quantity), 0) INTO v_loaded_sm
    FROM stock_movements sm
    WHERE sm.worker_id = p_worker_id AND sm.product_id = r.pid
      AND sm.movement_type = 'load' AND sm.created_at > v_anchor;

    IF v_loaded_sm > 0 THEN
