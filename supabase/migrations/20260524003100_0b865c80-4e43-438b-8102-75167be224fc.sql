...
BEGIN
  ...
  IF rec.worker_id IS NOT NULL AND rec.customer_id IS NOT NULL THEN
    ...
    -- Convert gift total pieces into the unit recorded on the offer so that
    -- gratuite_quantity is expressed in the same unit as gift_quantity_unit.
    -- Previously we always stored pieces but labelled the unit as 'box',
    -- which made displays multiply by pieces_per_box (e.g. 20 pieces shown as
    -- 20 boxes = 400 pieces).
...
-- Backfill: fix existing promo rows where gratuite_quantity was stored as
-- total pieces but gift_quantity_unit was set to 'box'. Divide by ppb so the
-- value matches the declared unit.
UPDATE public.promos p
SET gratuite_quantity = p.gratuite_quantity / GREATEST(pr.pieces_per_box, 1)
FROM public.products pr
WHERE pr.id = p.product_id
  AND p.gift_quantity_unit = 'box'
  AND pr.pieces_per_box > 1
  AND p.notes = 'هدية عرض مؤكدة'
  AND p.gratuite_quantity >= pr.pieces_per_box
  AND MOD(p.gratuite_quantity::integer, pr.pieces_per_box) = 0;