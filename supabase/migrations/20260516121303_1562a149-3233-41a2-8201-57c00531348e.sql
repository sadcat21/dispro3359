...
WITH CHECK (
  ...
  OR public.has_custom_role('store_manager')
  OR public.has_custom_role('warehouse_manager')
  OR public.get_user_role() = 'warehouse_manager'::public.app_role
...
USING (
  ...
  OR (
    manager_id = public.get_worker_id()
    AND status = ANY (ARRAY['pending'::text, 'rejected'::text, 'amended'::text, 'cancelled'::text])
...
WITH CHECK (
  ...
  OR (
    manager_id = public.get_worker_id()
    AND status = ANY (ARRAY['pending'::text, 'rejected'::text, 'amended'::text, 'approved'::text, 'cancelled'::text])
    AND frozen_at IS NULL
  )
  OR worker_id = public.get_worker_id()
);