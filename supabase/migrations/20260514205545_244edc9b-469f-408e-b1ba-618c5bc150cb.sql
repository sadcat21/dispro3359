...
BEGIN
  FOR r IN
    SELECT ws.product_id, ws.quantity AS current_qty, p.name AS product_name, COALESCE(p.pieces_per_box,1) AS ppb
    FROM worker_stock ws
    JOIN products p ON p.id = ws.product_id
    WHERE ws.worker_id = p_worker_id
  LOOP
    SELECT MAX(created_at) INTO v_last_load
    FROM stock_movements
    WHERE worker_id = p_worker_id AND product_id = r.product_id AND movement_type = 'load';

    IF v_last_load IS NULL THEN CONTINUE; END IF;

    SELECT COALESCE(SUM(quantity * r.ppb),0) INTO v_loaded
    FROM stock_movements
    WHERE worker_id = p_worker_id AND product_id = r.product_id
      AND movement_type = 'load' AND created_at = v_last_load;

    SELECT COALESCE(SUM(
      CASE WHEN movement_type IN ('delivery','direct_sale','promo_sale','promo_gift') THEN quantity ELSE 0 END
    ),0) INTO v_sold
    FROM stock_movements
    WHERE worker_id = p_worker_id AND product_id = r.product_id
      AND created_at > v_last_load;
    ...
    WHERE sm.worker_id = p_worker_id AND sm.product_id = r.product_id
