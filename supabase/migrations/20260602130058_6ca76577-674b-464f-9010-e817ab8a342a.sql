    ...
    SELECT COALESCE(SUM(
      CASE
        ...
        WHEN signed_quantity IS NOT NULL THEN
          CASE
            -- تعديل موجب (إرجاع للمخزون): يُحتسب فقط إذا كان هناك توصيل أصلي سالب لنفس الطلبية
            -- (وإلا يكون تعديلاً مباشراً على سجل التوصيل وحسابه يؤدي لتكرار).
