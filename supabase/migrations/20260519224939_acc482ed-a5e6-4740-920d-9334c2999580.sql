...
RETURNS TABLE(
  product_id uuid, product_name text, ppb integer, current_qty numeric, new_qty numeric,
  last_load_at timestamptz, loaded_pieces numeric, sold_pieces numeric, movements jsonb
)
...
BEGIN
  ...
  LOOP
    SELECT MAX(COALESCE(ls.completed_at, ls.created_at)) INTO v_fresh_load
