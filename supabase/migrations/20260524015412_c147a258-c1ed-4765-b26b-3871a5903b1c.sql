...
BEGIN
  ...
  LOOP
    ...
    -- Capture LIVE previous quantity at confirm-time (more accurate than the value
    -- captured at session-creation, which can be stale if sales/returns happened in between).
    ...
    IF v_worker_row_exists THEN
      -- Use LIVE previous quantity (not the stale recorded one) to decide reset vs accumulate.
    ...
    -- Sync recorded previous_quantity on the item to the LIVE value used,
    -- so audit/logs reflect the real prior balance at confirm-time.
