...
export const useNavigation = () => {
  ...
  const navItems = useMemo(() => {
    ...
    if (role === 'admin' || role === 'project_manager') {
      return {
        ...
        more: [
          ...
          { path: '/targets-leaderboard', icon: Trophy, label: 'لوحة أداء العمال' },
    ...
    if (activeRole?.custom_role_code === 'company_manager') {
      return {
        ...
        more: [
          ...
          { path: '/surplus-deficit', icon: Scale, label: t('nav.surplus_deficit') },
          { path: '/accounting', icon: Calculator, label: t('accounting.title') },
          { path: '/manager-accounting-review', icon: ClipboardCheck, label: t('admin_home.item.manager_accounting_review') },
    ...
    // Internal Supervisor — يتابع رحلة العامل: مسؤول المخزن → العملاء → مدير الفرع
