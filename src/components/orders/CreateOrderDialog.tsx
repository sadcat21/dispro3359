  ...
  const handleAddProductWithQuantity = (productId: string, quantity: number, giftInfo?: { giftQuantity: number; giftPieces: number; offerId?: string }, isUnitSale?: boolean, perItemPricing?: PerItemPricing) => {
    ...
    if (isUnitSale) {
      ...
      const newUnitItem = {
        ...
        itemInvoicePaymentMethod: perItemPricing?.invoicePaymentMethod ?? invoicePaymentMethod,
        itemInvoicePaymentSubType: perItemPricing?.invoicePaymentSubType ?? invoicePaymentSubType,
          ...
          <DialogHeader className="px-3 py-2 border-b shrink-0">
            {selectedCustomer ? (
              (() => {
                ...
                return (
                  <DialogTitle className="text-sm leading-tight pe-10">
