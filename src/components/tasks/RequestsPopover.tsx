...
const RequestsPopover: React.FC = () => {
  const { t } = useLanguage();
  const { tasks, incompleteTasks, updateTaskStatus, deleteTask } = useTasks('request');
      ...
      <Popover>
        ...
        <PopoverContent align="end" className="w-[min(96vw,20rem)] max-w-[96vw] p-0 h-[min(82dvh,42rem)] overflow-hidden flex flex-col">
          <StatusTabs
            ...
            t={t}
            title={t('requests.title')}
            translationPrefix="requests"
          />
        </PopoverContent>
      </Popover>
      <AddTaskDialog open={showAddDialog} onOpenChange={setShowAddDialog} taskType="request" />
    </>
  );
};
